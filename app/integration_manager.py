...
class IntegrationManager:
    ...
    def get_integration_function(self, integration_name):
        # Retrieve a callable integration function by name
        return self.integration_functions.get(integration_name)

# def get_integration_function(integration_name):
#     # Check if we're in an application context
#     if has_app_context():
#         # Retrieve the integration manager from the current app
#         integration_manager = current_app.integration_manager
#         # Get the integration function by name
#         return integration_manager.get_integration_function(integration_name)
#     else:
#         raise RuntimeError("get_integration_function must be called within an application context")

def get_integration_function(integration_name):
    if not has_app_context():
        raise RuntimeError("get_integration_function must be called within an application context")
    
    integration_manager = getattr(current_app, 'integration_manager', None)
    if integration_manager is None:
        raise AttributeError("IntegrationManager not initialized for the current app")
    
    integration_function = integration_manager.get_integration_function(integration_name)
    if integration_function is None:
        raise ValueError(f"No integration function found for '{integration_name}'")
