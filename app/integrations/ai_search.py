...
def generate_search_parameters(input_text):
    try:
        response = openai.chat.completions.create(
            model="gpt-4-turbo",
            messages=[
                {"role": "system", "content": """You are a helpful assistant expected to generate search parameters in a JSON array format for entities and relationships based on the given user input. Each parameter should be a dictionary with a "name" key. For example, for the input "Did Johnny Appleseed plant apple seeds?", your output should be: [{"name":"John"},{"name":"Appleseed"},{"name":"Apple"},{"name":"Seed"}]"""},
                {"role": "user", "content": f"User input: {input_text}"}
            ]
        )
        search_parameters = response.choices[0].message.content
        print("Raw search parameters:", search_parameters)
        
        # Extract the JSON array from the response
        json_match = re.search(r'\[.*\]', search_parameters, re.DOTALL)
        if json_match:
            json_str = json_match.group(0)
            # Parse the JSON string
            parsed_parameters = json.loads(json_str)
            print("Parsed search parameters:", parsed_parameters)
